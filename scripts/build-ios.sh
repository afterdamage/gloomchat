...
[ -n "${GLOOMCHAT_NEW_GROUP}" ] && {
	# App group IDs
	sed -i "" "s/group.${GLOOMCHAT_ORIG_GROUP}.app/group.${GLOOMCHAT_NEW_GROUP}.app/g" "ios/GloomChat Share/GloomChat Share.entitlements"
	sed -i "" "s/group.${GLOOMCHAT_ORIG_GROUP}.app/group.${GLOOMCHAT_NEW_GROUP}.app/g" "ios/Runner/Runner.entitlements"
	sed -i "" "s/group.${GLOOMCHAT_ORIG_GROUP}.app/group.${GLOOMCHAT_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
	# Bundle identifiers
	sed -i "" "s/${GLOOMCHAT_ORIG_GROUP}.app/${GLOOMCHAT_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
}

[ -n "${GLOOMCHAT_NEW_TEAM}" ] && {
	# Code signing team
	sed -i "" "s/DEVELOPMENT_TEAM = .*;/DEVELOPMENT_TEAM = ${GLOOMCHAT_NEW_TEAM};/g" "ios/Runner.xcodeproj/project.pbxproj"
}
cat << EOHELP
...
2. Fix it yourself
    ...
    - Ensure that Xcode created the App ID successfully (for gloomchat.app and gloomchat.app.GloomChat-Share)
...
[ -n "${GLOOMCHAT_INSTALL_IPA}" ] && {
  ...
  cfgutil --foreach install-app "${TMPDIR}/gloomchat.ipa"
  rm -rf "${TMPDIR}"
}
