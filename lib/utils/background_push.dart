...
class BackgroundPush {
  ...
  Future<void> _setupApns() async {
    ...
    String? token;
    try {
      token = await channel.invokeMethod<String>('requestToken');
      if (token == null) throw 'APNs token is null';
    } catch (e, s) {
      Logs().w('[Push] Cannot get APNs token', e, e is String ? null : s);
      await _noFcmWarning();
      return;
    }
    await setupPusher(
  ...
  Future<void> setupUp() async {
    await UnifiedPushUi(
      ...
      onNoDistribDialogDismissed: _noFcmWarning,
