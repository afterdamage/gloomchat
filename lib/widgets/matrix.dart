...
class MatrixState extends State<Matrix> with WidgetsBindingObserver {
  ...
  String? get activeRoomId {
    final route = GloomChatApp.router.routeInformationProvider.value.uri.path;
        ...
        .listen((KeyVerification request) async {
          ...
          request.onUpdate = () {
            if (!hidPopup &&
              ...
              GloomChatApp.router.pop('dialog');
            }
            hidPopup = true;
          };
          ...
          await KeyVerificationDialog(request: request).show(
            GloomChatApp.router.routerDelegate.navigatorKey.currentContext ??
                context,
          );
        });
    onLoginStateChanged[name] ??= c.onLoginStateChanged.stream.listen((state) {
      ...
      if (loggedInWithMultipleClients && state != LoginState.loggedIn) {
        ...
        if (state != LoginState.loggedIn) {
          GloomChatApp.router.go('/rooms');
        }
      } else {
        GloomChatApp.router.go(
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    // Keep sync running during brief 'inactive' interruptions (notification
    // bar, incoming system call, lock-screen glance on iOS). Stopping sync
    // on 'inactive' means a call invite arriving in that window gets missed
    // until the app fully resumes — by then the caller may have hung up.
    final syncEnabled =
    ...
    for (final client in widget.clients) {
      ...
      if (PlatformInfos.isMobile) {
        client.backgroundSync = syncEnabled;
        client.requestHistoryOnLimitedTimeline = !syncEnabled;
        Logs().v('Set background sync to $syncEnabled (lifecycle: $state)');
      }
    }
  }
  ...
  Future<void> dehydrateAction(BuildContext context) async {
    ...
    final exportFileName =
        'gloomchat-export-${DateFormat(DateFormat.YEAR_MONTH_DAY).format(DateTime.now())}.gloombackup';
